...
CREATE TABLE IF NOT EXISTS laboral_information(
    ...
    period TEXT NOT NULL DEFAULT "NA",
...
CREATE TABLE IF NOT EXISTS formation_as_auditor_info(
    id INTEGER PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
    course_name TEXT NOT NULL DEFAULT 'NA',
    date TEXT NOT NULL DEFAULT 'NA',
    score INTEGER DEFAULT 0
);
...
CREATE TABLE IF NOT EXISTS formation_info(
    ...
    formation_as_auditor_info_id INTEGER REFERENCES formation_as_auditor_info(id)
);

CREATE TABLE IF NOT EXISTS audit_info(
    id INTEGER PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
    appoitment TEXT NOT NULL DEFAULT 'NA',
    date TEXT NOT NULL DEFAULT 'NA',
    score INTEGER DEFAULT 0
);
...
CREATE TABLE IF NOT EXISTS audit_experience_info(
    ...
    audit_experience_info_id INTEGER REFERENCES audit_info(id)
);
...
CREATE TABLE IF NOT EXISTS audit_cal(
    ...
    date TEXT NOT NULL CHECK( date <> ''),
    total_score INTEGER,
    quality_management_experience_id INTEGER REFERENCES quality_management_experience(id),
    formation_as_auditor_id INTEGER REFERENCES formation_as_auditor(id),
    audit_experience_id INTEGER REFERENCES audit_experience(id),
